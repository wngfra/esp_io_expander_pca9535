...
static const char *TAG = "pca9535 test";
...
static void i2c_dev_pca9535_init(void)
{
    esp_err_t ret = esp_io_expander_new_i2c_pca9535(i2c_handle, I2C_ADDRESS, &io_expander);
    TEST_ASSERT_EQUAL_MESSAGE(ESP_OK, ret, "PCA9535 create returned error");
}

static void i2c_dev_pca9535_deinit(void)
{
    esp_err_t ret = esp_io_expander_del(io_expander);
    TEST_ASSERT_EQUAL_MESSAGE(ESP_OK, ret, "PCA9535 delete returned error");
}

TEST_CASE("IO expander pca9535 test", "[pca9535]")
{
    i2c_bus_init();
    i2c_dev_pca9535_init();
